...
const fs = require('fs');
const template = fs.readFileSync ('./templates/backorder.html').toString();
const { TemplateParser , RenderContext} = require('./teamplate');
...
function newStyle ( req){
    const values ={};
    req.deliveries[0].fields.forEach( item=>{
...
async function test (){
    ...
    let t = new TemplateParser({body:template}).parse();
    let values = newStyle(req2);
    let context = new RenderContext({values:values});
    
    let out = t.render(context);
    console.log(out);
}
